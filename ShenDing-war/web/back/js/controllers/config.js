...
app.controller('ConfigAdLimitController', ['$scope', '$http', '$modal', '$location', "$state", function ($scope, $http, $modal, $location, $state) {
        $scope.list = null;
        $scope.listLoading = false;
        $scope.listLoadingData = false;
        $scope.order = {show: false, submitting: false};
        ...
        $scope.getList = function (page, isInit) {
            ...
            $http.get("/webservice/product/ad_limit_config_list?type=1&pageIndex=" + $scope.currentPage + "&search=" + $scope.listSearch).success(function (responseData) {
                if (responseData.success !== "1") {
                    $.scojs_message(responseData.msg, $.scojs_message.TYPE_ERROR);
                    if (responseData.success == "-1") {
                        $state.go('access.signin');
        ...
        $scope.submitForm = function () {
            $scope.order.submitting = true;
            $scope.order.type = 1;
            $http.post("/webservice/product/save_ad_limit_config", $scope.order).success(function (responseData) {
                if (responseData.success !== "1") {
                    $.scojs_message(responseData.msg, $.scojs_message.TYPE_ERROR);
                    if (responseData.success == "-1") {
                        $state.go('access.signin');
