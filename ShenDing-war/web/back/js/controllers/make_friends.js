'use strict';
/* Controllers */
// make_friends controller
app.controller('MFSeekInfoController', ['$scope', '$http', '$modal', '$location', "$state", "$stateParams", function ($scope, $http, $modal, $location, $state, $stateParams) {
        $scope.seek = null;
...
app.controller('MFGoodsController', ['$scope', '$http', '$modal', '$location', "$state", function ($scope, $http, $modal, $location, $state) {
        $scope.list = null;
        ...
        $scope.submitForm = function () {
            ...
            $http.post("/webservice/admin/create_update_goods", {category: "MAKE_FRIENDS", id: $scope.goods.id, peopleCount: $scope.goods.peopleCount, name: $scope.goods.name, type: $scope.goods.type, price: $scope.goods.price, uid: $scope.checkedEle.id, province: $scope.province, weChatCode: $scope.goods.weChatCode, qqCode: $scope.goods.qqCode}).success(function (responseData) {
                if (responseData.success !== "1") {
                    $.scojs_message(responseData.msg, $.scojs_message.TYPE_ERROR);
                    if (responseData.success == "-1") {
                        $state.go('access.signin');
...
app.controller('MFOrderController', ['$scope', '$http', '$modal', '$location', "$state", function ($scope, $http, $modal, $location, $state) {
        $scope.list = null;
        $scope.listLoading = false;
        $scope.listLoadingData = false;
        $scope.listSearch = "";
        ...
        $scope.getList($scope.currentPage, false);
        $scope.add = function () {
            $state.go('app.make_friends.create_order');
        };
        $scope.deleteItems = function () {
            var checkedList = new Array();
            for (var i = 0; i < $scope.list.length; i++) {
...
app.controller('MFContractListController', ['$scope', '$http', '$modal', '$location', "$state", function ($scope, $http, $modal, $location, $state) {
        $scope.list = null;
        $scope.listLoading = false;
        $scope.listLoadingData = false;
        $scope.listSearch = "";
        ...
        $scope.getList = function (page, isInit) {
            ...
            $http.get("/webservice/admin/order_list?contract=true&category=MAKE_FRIENDS&pageIndex=" + $scope.currentPage + "&search=" + $scope.listSearch).success(function (responseData) {
                if (responseData.success !== "1") {
                    $.scojs_message(responseData.msg, $.scojs_message.TYPE_ERROR);
                    if (responseData.success == "-1") {
                        $state.go('access.signin');
        ...
        $scope.downList = function () {
            window.open("/admin/contract_list?category=MAKE_FRIENDS");
        };
...
app.controller('MFCreateOrderController', ['$scope', '$http', '$modal', '$location', "$state", "$stateParams", function ($scope, $http, $modal, $location, $state, $stateParams) {
        ...
        $scope.searchSaleGoods = function () {
            ...
            $http.get("/webservice/admin/goods_list?search=" + $scope.searchAgentMsg + "&status=SALE&category=MAKE_FRIENDS").success(function (responseData) {
                if (responseData.success !== "1") {
                    $.scojs_message(responseData.msg, $.scojs_message.TYPE_ERROR);
                    if (responseData.success == "-1") {
                        $state.go('access.signin');
        ...
        $scope.checkAgent = function (ele) {
            ...
            if (ele.type == 'GOVERNMENT_DIRECTLY' || ele.type == 'PROVINCIAL_CAPITAL') {
                $scope.order.divideAmount = 1200;
            } else if (ele.type == 'HOT' || ele.type == 'PREFECTURE') {
                $scope.order.divideAmount = 1000;
            } else {
                $scope.order.divideAmount = 800;
            }
        };
            ...
            if (payDateTime != "" && ($scope.order.amount == null || $scope.order.amount == "")) {
                ...
                $http.post("/webservice/admin/create_update_order", {category: "MAKE_FRIENDS", peopleCountFee: $scope.order.peopleCountFee, divideUserId: $scope.checkedDivideUser.id, userAmount: $scope.order.userAmount, remark: $scope.order.remark, id: $scope.order.id, divideAmount: $scope.order.divideAmount, backAmount: $scope.order.backAmount, price: $scope.order.price, amount: $scope.order.amount, orderRecordType: $scope.order.amountType, gatewayType: $scope.order.payType, payDate: payDateTime, goodsId: $scope.checkedAgent.id, recommendIds: recommendIds, recommendOrderIds: recommendOrderIds, rates: $scope.rate}).success(function (responseData) {
                    if (responseData.success !== "1") {
                        ...
                        $.scojs_message(responseData.msg, $.scojs_message.TYPE_OK);
                        $state.go('app.make_friends.order');
                    }
                    $scope.order.submitting = false;
                });
            }
...
app.controller('MFUpdateOrderController', ['$scope', '$http', '$modal', '$location', "$state", "$stateParams", function ($scope, $http, $modal, $location, $state, $stateParams) {
        $scope.order = null;
        ...
        $scope.submitForm = function () {
            ...
            if (payDateTime != "" && ($scope.orderRecord.amount == null || $scope.orderRecord.amount < 0)) {
                ...
                $http.post("/webservice/admin/create_order_record", {category: "MAKE_FRIENDS", peopleCountFee: $scope.peopleCountFee, orderId: $scope.orderId, amount: $scope.orderRecord.amount, orderRecordType: $scope.orderRecord.amountType, gatewayType: $scope.orderRecord.payType, payDate: payDateTime}).success(function (responseData) {
                    if (responseData.success !== "1") {
                        $.scojs_message(responseData.msg, $.scojs_message.TYPE_ERROR);
                        if (responseData.success == "-1") {
                            $state.go('access.signin');
...
app.controller('MFAdController', ['$scope', '$http', '$modal', '$location', "$state", function ($scope, $http, $modal, $location, $state) {
        $scope.list = null;
        $scope.listLoading = false;
        $scope.listLoadingData = false;
        $scope.listSearch = "";
        ...
        $scope.getList = function (page, isInit) {
            ...
            $http.get("/webservice/admin/ad_list?pageIndex=" + $scope.currentPage + "&search=" + $scope.listSearch + "&category=MAKE_FRIENDS").success(function (responseData) {
                if (responseData.success !== "1") {
                    $.scojs_message(responseData.msg, $.scojs_message.TYPE_ERROR);
                    if (responseData.success == "-1") {
                        $state.go('access.signin');
        ...
        $scope.getList($scope.currentPage, false);
        $scope.add = function () {
            $state.go('app.make_friends.create_ad');
        };
        $scope.deleteItems = function () {
            var checkedList = new Array();
            for (var i = 0; i < $scope.list.length; i++) {
...
app.controller('MFCreateAdController', ['$scope', '$http', '$modal', '$location', "$state", "$stateParams", function ($scope, $http, $modal, $location, $state, $stateParams) {
        ...
        $scope.searchGoodsList = function () {
            ...
            $http.get("/webservice/admin/goods_list?status=SOLD_OUT&category=MAKE_FRIENDS&search=" + $scope.searchGoodsMsg).success(function (responseData) {
                if (responseData.success !== "1") {
                    $.scojs_message(responseData.msg, $.scojs_message.TYPE_ERROR);
                    if (responseData.success == "-1") {
                        $state.go('access.signin');
        ...
        $scope.submitForm = function () {
            ...
            $scope.ad.category = "MAKE_FRIENDS";
            ...
            $http.post("/webservice/admin/create_update_new_ad", $scope.ad).success(function (responseData) {
                ...
                if (responseData.success !== "1") {
                    ...
                    $.scojs_message(responseData.msg, $.scojs_message.TYPE_OK);
                    $state.go('app.make_friends.new_ad_list');
                }
            });
        };
        $scope.backPage = function () {
...
app.controller('MFOrderUserPriceController', ['$scope', '$http', '$modal', '$location', "$state", function ($scope, $http, $modal, $location, $state) {
        $scope.goodsTypeList = null
        ...
        $scope.loadDic = function () {
            ...
            $http.get("/webservice/admin/dic_def_user_amount?category=MAKE_FRIENDS&type=" + $scope.dic.type).success(function (responseData) {
                if (responseData.success !== "1") {
                    $.scojs_message(responseData.msg, $.scojs_message.TYPE_ERROR);
                    if (responseData.success == "-1") {
                        $state.go('access.signin');
        ...
        $scope.submitForm = function () {
            $scope.dic.submitting = true;
            $http.post("/webservice/admin/dic_def_user_amount", {type: $scope.dic.type, amount: $scope.dic.amount, category: "MAKE_FRIENDS"}).success(function (responseData) {
                if (responseData.success !== "1") {
                    $.scojs_message(responseData.msg, $.scojs_message.TYPE_ERROR);
                    if (responseData.success == "-1") {
                        $state.go('access.signin');
...
app.controller('MFUpdateAdController', ['$scope', '$http', '$modal', '$location', "$state", "$stateParams", function ($scope, $http, $modal, $location, $state, $stateParams) {
        $scope.ad = null;
...
app.controller('MFUserWageLogListController', ['$scope', '$http', '$modal', '$location', "$state", function ($scope, $http, $modal, $location, $state) {
        $scope.list = null;
        $scope.listLoading = false;
        $scope.listLoadingData = false;
        $scope.search = "";
        ...
        $scope.getList = function (page, isInit) {
            ...
            $http.get("/webservice/admin/user_wage_log_list?pageIndex=" + $scope.currentPage + "&category=MAKE_FRIENDS&search=" + $scope.search + "&start=" + start + "&end=" + end).success(function (responseData) {
                if (responseData.success !== "1") {
                    $.scojs_message(responseData.msg, $.scojs_message.TYPE_ERROR);
                    if (responseData.success == "-1") {
                        $state.go('access.signin');
        ...
        $scope.wageTotalList = function () {
            ...
            window.open("/admin/USER_WAGE_LOG_TOTAL_LIST?startDate=" + start + "&category=MAKE_FRIENDS&endDate=" + end);
        };
        ...
        $scope.wageLogList = function () {
            ...
            window.open("/admin/user_wage_log_list?startDate=" + start + "&category=MAKE_FRIENDS&endDate=" + end);
        };
        ...
        $scope.orderRecordList = function () {
            ...
            window.open("/admin/order_record_list?startDate=" + start + "&category=MAKE_FRIENDS&endDate=" + end);
        };

    }]);


app.controller('MFPlaceUserController', ['$scope', '$http', '$modal', '$location', "$state", function ($scope, $http, $modal, $location, $state) {
        $scope.provinceList = null;
        $scope.province = null;
        $scope.goods = {submitting: false};
        $scope.searchUserMsg = "";
        ...
        $scope.submitForm = function () {
            $scope.goods.submitting = true;
            $http.post("/webservice/admin/modify_place_user", {uid: $scope.checkedEle.id, province: $scope.province, category: "MAKE_FRIENDS"}).success(function (responseData) {
                if (responseData.success !== "1") {
                    $.scojs_message(responseData.msg, $.scojs_message.TYPE_ERROR);
                    if (responseData.success == "-1") {
                        $state.go('access.signin');
...
app.controller('MFOrderWageController', ['$scope', '$http', '$modal', '$location', "$state", function ($scope, $http, $modal, $location, $state) {
        $scope.list = null;
        $scope.listLoading = false;
        $scope.listLoadingData = false;
        $scope.search = "";
        ...
        $scope.getList = function (page, isInit) {
            ...
            $http.get("/webservice/admin/order_wage_list?pageIndex=" + $scope.currentPage + "&category=MAKE_FRIENDS&search=" + $scope.search + "&start=" + start + "&end=" + end).success(function (responseData) {
                if (responseData.success !== "1") {
                    $.scojs_message(responseData.msg, $.scojs_message.TYPE_ERROR);
                    if (responseData.success == "-1") {
                        $state.go('access.signin');
        ...
        $scope.adList = function () {
            ...
            window.open("/admin/wage_log_list?startDate=" + start + "&category=MAKE_FRIENDS&endDate=" + end);
        };
        ...
        $scope.wageList = function () {
            ...
            window.open("/admin/order_wage_list?startDate=" + start + "&category=MAKE_FRIENDS&endDate=" + end);
        };

    }]);

app.controller('MFWageController', ['$scope', '$http', '$modal', '$location', "$state", function ($scope, $http, $modal, $location, $state) {
        $scope.list = null;
        $scope.listLoading = false;
        $scope.listLoadingData = false;
        $scope.search = "";
        ...
        $scope.getList = function (page, isInit) {
            ...
            $http.get("/webservice/admin/wage_list?pageIndex=" + $scope.currentPage + "&category=MAKE_FRIENDS&search=" + $scope.search + "&start=" + start + "&end=" + end).success(function (responseData) {
                if (responseData.success !== "1") {
                    $.scojs_message(responseData.msg, $.scojs_message.TYPE_ERROR);
                    if (responseData.success == "-1") {
                        $state.go('access.signin');
        ...
        $scope.adList = function () {
            ...
            window.open("/admin/ad_list?startDate=" + start + "&category=MAKE_FRIENDS&endDate=" + end);
        };
        ...
        $scope.wageList = function () {
            ...
            window.open("/admin/wage_list?startDate=" + start + "&category=MAKE_FRIENDS&endDate=" + end);
        };
        ...
        $scope.wageTotalList = function () {
            ...
            window.open("/admin/wage_total_list?startDate=" + start + "&category=MAKE_FRIENDS&endDate=" + end);
        };
